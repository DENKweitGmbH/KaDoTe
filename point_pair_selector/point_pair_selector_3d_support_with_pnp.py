...
"""GUI for creating camera calibration and position configurations."""
...
try:
    ...
    messagebox.showerror(
        "Error", "OpenCV library not found. Please install it using: pip install opencv-python"
    )
    sys.exit()
try:
    ...
    messagebox.showerror(
        "Error", "Matplotlib library not found. Please install it using: pip install matplotlib"
    )
    sys.exit()
try:
    ...
    messagebox.showerror(
        "Error", "Open3D library not found. Please install it using: pip install open3d"
    )
...
class PointPairSelector:
    def __init__(self, root) -> None:
        ...
        self.tk_blended_image_display = (
            None  # PhotoImage for alignment display (potentially zoomed)
        )
        ...
        self.alignment_window_active_pc_slice_xyz = (
            None  # X,Y,Z data for current PC slice in alignment view
        )
        self.alignment_current_selection_index = (
            None  # Index of selected point in alignment listbox
        )
        ...
        self.heatmap_yz_min_max_b = (
            None  # To store (y_min, y_max, z_min, z_max) of data used for heatmap
        )
        self.last_filtered_points_for_heatmap_xyz = (
            None  # Store X,Y,Z of points used for last heatmap
        )
    ...
    def create_gui(self) -> None:
        ...
        self.btn_zoom_in_a = Button(
            controls_frame_a, text="+", width=2, command=lambda: self.zoom("a", ZOOM_STEP)
        )
        self.btn_zoom_in_a.pack(side=LEFT)
        self.btn_zoom_out_a = Button(
            controls_frame_a, text="-", width=2, command=lambda: self.zoom("a", 1 / ZOOM_STEP)
        )
        self.btn_zoom_out_a.pack(side=LEFT)
        self.btn_zoom_reset_a = Button(
            controls_frame_a, text="100%", command=lambda: self.zoom("a", 1.0, reset=True)
        )
        ...
        self.btn_flip_h_a = Button(
            controls_frame_a, text="Flip V", command=self.flip_image_a_horizontal
        )
        self.btn_flip_h_a.pack(side=LEFT, padx=2)
        self.btn_flip_v_a = Button(
            controls_frame_a, text="Flip H", command=self.flip_image_a_vertical
        )
        self.btn_flip_v_a.pack(side=LEFT, padx=2)
        Label(controls_frame_a, text="(Click first)").pack(
            side=LEFT, padx=(10, 0)
        )  # Adjusted padding
        ...
        self.canvas_a.bind(
            "<ButtonPress-2>", lambda event: self.canvas_a.scan_mark(event.x, event.y)
        )
        self.canvas_a.bind(
            "<B2-Motion>", lambda event: self.canvas_a.scan_dragto(event.x, event.y, gain=1)
        )
        ...
        self.btn_zoom_in_b = Button(
            controls_frame_b, text="+", width=2, command=lambda: self.zoom("b", ZOOM_STEP)
        )
        self.btn_zoom_in_b.pack(side=LEFT)
        self.btn_zoom_out_b = Button(
            controls_frame_b, text="-", width=2, command=lambda: self.zoom("b", 1 / ZOOM_STEP)
        )
        self.btn_zoom_out_b.pack(side=LEFT)
        self.btn_zoom_reset_b = Button(
            controls_frame_b, text="100%", command=lambda: self.zoom("b", 1.0, reset=True)
        )
        self.btn_zoom_reset_b.pack(side=LEFT, padx=2)
        Label(controls_frame_b, text="(Click second)").pack(
            side=LEFT, padx=(10, 5)
        )  # Adjusted padding
        ...
        self.pc_b_colormap_menu = OptionMenu(
            controls_frame_b,
            self.pc_b_colormap_var,
            *self.available_colormaps,
            command=self._on_pc_b_colormap_change,
        )
        self.pc_b_colormap_menu.pack(side=LEFT, padx=2)

        self.btn_view_3d_pc_b = Button(
            controls_frame_b, text="View 3D", command=self.view_3d_point_cloud_b, state=DISABLED
        )
        ...
        self.pc_b_x_min_slider = tk.Scale(
            pc_controls_frame_b,
            from_=0,
            to=1,
            resolution=0.01,
            orient=HORIZONTAL,
            variable=self.pc_b_x_min_val,
            command=self._on_pc_b_x_range_change,
            length=100,
            state=DISABLED,
        )
        self.pc_b_x_min_slider.pack(side=LEFT, padx=2)
        Label(pc_controls_frame_b, textvariable=self.pc_b_x_min_val).pack(
            side=LEFT, padx=2
        )  # Show value

        self.pc_b_x_max_slider = tk.Scale(
            pc_controls_frame_b,
            from_=0,
            to=1,
            resolution=0.01,
            orient=HORIZONTAL,
            variable=self.pc_b_x_max_val,
            command=self._on_pc_b_x_range_change,
            length=100,
            state=DISABLED,
        )
        self.pc_b_x_max_slider.pack(side=LEFT, padx=2)
        Label(pc_controls_frame_b, textvariable=self.pc_b_x_max_val).pack(
            side=LEFT, padx=2
        )  # Show value

        self.btn_update_heatmap_b = Button(
            pc_controls_frame_b,
            text="Update Heatmap",
            command=self._trigger_heatmap_generation_b,
            state=DISABLED,
        )
        ...
        self.canvas_b.bind(
            "<ButtonPress-2>", lambda event: self.canvas_b.scan_mark(event.x, event.y)
        )
        self.canvas_b.bind(
            "<B2-Motion>", lambda event: self.canvas_b.scan_dragto(event.x, event.y, gain=1)
        )
        ...
        self.btn_save_points = Button(
            action_buttons_frame, text="Save Points", command=self.save_point_pairs, state=DISABLED
        )
        self.btn_save_points.pack(pady=(5, 2), fill=X)
        self.btn_load_points = Button(
            action_buttons_frame, text="Load Points", command=self.load_point_pairs, state=NORMAL
        )  # Initially enabled
        ...
        self.chk_show_all_points = Checkbutton(
            vis_frame,
        ...
        Radiobutton(
            warp_method_frame,
            text=WARP_METHOD_HOMOGRAPHY,
            variable=self.warp_method,
            value=WARP_METHOD_HOMOGRAPHY,
            command=self._invalidate_warp,
        ).pack(anchor=W)
        Radiobutton(
            warp_method_frame,
            text=WARP_METHOD_TPS,
            variable=self.warp_method,
            value=WARP_METHOD_TPS,
            command=self._invalidate_warp,
        ).pack(anchor=W)

        # Buttons for calculation and showing alignment
        self.btn_calculate_warp = Button(
            warp_frame, text="Calculate Warp", command=self.calculate_warp, state=DISABLED
        )  # Renamed command
        self.btn_calculate_warp.pack(pady=(10, 2), fill=X, padx=5)

        self.btn_show_alignment = Button(
            warp_frame,
            text="Show Warped Alignment",  # Renamed text
            command=self.show_alignment,
            state=DISABLED,
        )
        self.btn_show_alignment.pack(pady=2, fill=X, padx=5)

        self.btn_save_homography = Button(
            warp_frame, text="Save Homography", command=self.save_homography_data, state=DISABLED
        )
        self.btn_save_homography.pack(pady=(5, 2), fill=X, padx=5)
        self.btn_load_homography = Button(
            warp_frame, text="Load Homography", command=self.load_homography_data, state=NORMAL
        )  # Initially enabled
        self.btn_load_homography.pack(pady=2, fill=X, padx=5)

        self.btn_save_cam_pos = Button(
            warp_frame, text="Save Camera Position", command=self.save_cam_pos_data, state=DISABLED
        )
    ...
    def _update_zoom_button_states(self) -> None:
        ...
        if self.btn_zoom_in_a:
            self.btn_zoom_in_a.config(state=state_a)
        if self.btn_zoom_out_a:
            self.btn_zoom_out_a.config(state=state_a)
        if self.btn_zoom_reset_a:
            self.btn_zoom_reset_a.config(state=state_a)
        if self.btn_rotate_a:
            self.btn_rotate_a.config(state=state_a)
        if self.btn_flip_h_a:
            self.btn_flip_h_a.config(state=state_a)
        if self.btn_flip_v_a:
            self.btn_flip_v_a.config(state=state_a)

        if self.btn_zoom_in_b:
            self.btn_zoom_in_b.config(state=state_b)
        if self.btn_zoom_out_b:
            self.btn_zoom_out_b.config(state=state_b)
        if self.btn_zoom_reset_b:
            self.btn_zoom_reset_b.config(state=state_b)

        # Point cloud controls for B
        pc_state = (
            NORMAL if self.is_b_point_cloud and self.point_cloud_data_b is not None else DISABLED
        )
        if self.pc_b_x_min_slider:
            self.pc_b_x_min_slider.config(state=pc_state)
        if self.pc_b_x_max_slider:
            self.pc_b_x_max_slider.config(state=pc_state)
        if self.btn_update_heatmap_b:
            self.btn_update_heatmap_b.config(state=pc_state)
        if self.pc_b_colormap_menu:
            self.pc_b_colormap_menu.config(state=pc_state)
        if self.btn_view_3d_pc_b:
            self.btn_view_3d_pc_b.config(state=pc_state)

    def _update_warp_button_states(self) -> None:  # Renamed
        ...
        if self.btn_save_homography:
            self.btn_save_homography.config(
                state=NORMAL if self.homography_matrix is not None else DISABLED
            )
        ...
        if self.btn_save_points:
            self.btn_save_points.config(state=NORMAL if has_points else DISABLED)
        if self.btn_remove_pair:
            self.btn_remove_pair.config(
                state=NORMAL
                if self.current_selection_index is not None and has_points
                else DISABLED
            )

    def _invalidate_warp(self) -> None:
        ...
        if target == "b" and file_extension == ".npy":
            try:
                ...
                if self.point_cloud_data_b.ndim != 2 or self.point_cloud_data_b.shape[1] != 3:
                    msg = "Point cloud data must be an N x 3 array."
                ...
                self.set_status(
                    f"Point cloud {os.path.basename(file_path)} loaded ({self.point_cloud_data_b.shape[0]} points). Adjust X-range for YZ projection."
                )
                ...
                messagebox.showerror(
                    "Point Cloud Load Error", f"Failed to load point cloud {file_path}.\nError: {e}"
                )
                self.set_status("Error loading Point Cloud B.")
            ...
            try:
                ...
                self.set_status(
                    f"{os.path.basename(file_path)} loaded ({image.width}x{image.height})"
                )
                self._update_zoom_button_states()
                self._update_warp_button_states()
                return True
                ...
                messagebox.showerror(
                    "Image Load Error", f"Failed to load image {file_path}.\nError: {e}"
                )
    ...
    def flip_image_a_horizontal(self) -> None:
        if not self.image_a:
            return
    ...
    def flip_image_a_vertical(self) -> None:
        if not self.image_a:
            return
    ...
    def _get_transformed_image_a_for_display(self):
        ...
        return self._apply_transformations_to_pil_image_a(img_to_transform)

    def _update_display_image(self, target) -> None:
        ...
        try:
            ...
            display_image = original_image_for_display.resize(
                (new_width_zoomed, new_height_zoomed), resample_filter
            )
            ...
            canvas.delete("all")
            canvas.create_image(
                0, 0, anchor=NW, image=tk_image, tags="image"
            )  # Displayed image is already transformed and zoomed
            ...
            messagebox.showwarning(
                "Zoom Error", f"Could not resize image {target.upper()} at this zoom level.\n{e}"
            )
            if canvas:
                canvas.delete("all")
                canvas.config(scrollregion=(0, 0, 0, 0))
    ...
    def zoom(self, target, factor, reset=False) -> None:
    ...
    def _on_pc_b_x_range_change(self, value) -> None:
        ...
        self.set_status(
            f"X-Range for YZ projection: [{self.pc_b_x_min_val.get():.2f}, {self.pc_b_x_max_val.get():.2f}]. Click 'Update Heatmap'."
        )

    def _on_pc_b_colormap_change(self, selected_colormap_name) -> None:
    ...
    def _trigger_heatmap_generation_b(self) -> None:
        ...
        if self.heatmap_image_b:
            ...
            self.set_status(
                f"YZ heatmap generated and displayed. X-Range: [{self.pc_b_x_min_val.get():.2f}, {self.pc_b_x_max_val.get():.2f}]"
            )
        else:
            if self.canvas_b:
                self.canvas_b.delete("all")
    ...
    def _generate_heatmap_pil_image_b(self):
        ...
        x_idx, _y_idx, _z_idx = (2, 1, 0) if is_fortran_order else (0, 1, 2)
        ...
        heatmap, _, _ = np.histogram2d(
            y_coords_for_hist,
            z_coords_for_hist,
            bins=(HEATMAP_WIDTH, HEATMAP_HEIGHT),
            range=hist_range,
        )
        ...
        heatmap_norm = (heatmap - np.min(heatmap)) / (
            np.max(heatmap) - np.min(heatmap) + 1e-9
        )  # Avoid div by zero
        colored_heatmap = current_colormap(heatmap_norm)  # RGBA

        # Convert to PIL Image
        return Image.fromarray((colored_heatmap[:, :, :3] * 255).astype(np.uint8), "RGB")

    def view_3d_point_cloud_b(self) -> None:
        ...
        try:
            ...
            messagebox.showerror(
                "3D View Error", "Open3D library is not available. Cannot display 3D point cloud."
            )
            return
        ...
        try:
            ...
            if is_fortran_order:  # Original was [z,y,x]
                points_for_o3d = filtered_pc_data[
                    :, [x_idx_orig, y_idx_orig, z_idx_orig]
                ]  # This is already [x,y,z] effectively after selection
            else:  # Original was [x,y,z]
                points_for_o3d = filtered_pc_data

            pcd.points = o3d.utility.Vector3dVector(
                points_for_o3d[:, [0, 1, 2]]
            )  # Ensure we take the first 3 columns if more exist
            ...
            if len(x_values_for_coloring) > 0:
                min_x_color, max_x_color = (
                    np.min(x_values_for_coloring),
                    np.max(x_values_for_coloring),
                )
                if max_x_color > min_x_color:  # Avoid division by zero
                    normalized_x_color = (x_values_for_coloring - min_x_color) / (
                        max_x_color - min_x_color
                    )
                    selected_colormap_name = self.pc_b_colormap_var.get()
            ...
            o3d.visualization.draw_geometries(
                [pcd],
                window_name=f"Point Cloud B (X-Range: [{x_min_filter:.2f}, {x_max_filter:.2f}]) - 3D View",
            )
    ...
    def _get_heatmap_pixel_from_original_yz(self, original_y, original_z):
        ...
        if y_max_data == y_min_data:
            ...
            pixel_x = ((original_y - y_min_data) / (y_max_data - y_min_data)) * (
                HEATMAP_WIDTH - 1.0
            )
        ...
        if z_max_data == z_min_data:
            ...
            pixel_y = ((z_max_data - original_z) / (z_max_data - z_min_data)) * (
                HEATMAP_HEIGHT - 1.0
            )
    ...
    def _transform_coords_from_display_to_original_a(self, display_x, display_y):
        ...
        return round(x_original), round(y_original)
    ...
    def _get_original_coords(self, target, event):
        ...
        if (
            0 <= canvas_x < current_display_image.width
            and 0 <= canvas_y < current_display_image.height
        ):
            ...
            # Target B (no visual transformations other than zoom)
            return int(unzoomed_x), int(unzoomed_y)
        return None

    def on_canvas_a_click(self, event) -> None:
    ...
    def on_canvas_b_click(self, event) -> None:
        if not self.image_b:
        ...
        if coords:  # Coords are pixel coordinates on the displayed image/heatmap
            ...
            if self.is_b_point_cloud:
                if (
                    self.heatmap_yz_min_max_b is None
                    or self.last_filtered_points_for_heatmap_xyz is None
                ):
                    self.set_status(
                        "Heatmap data or filtered point cloud slice not ready for point selection."
                    )
                    return
                ...
                original_yz_from_click = self._get_original_yz_from_heatmap_pixel(
                    coords[0], coords[1]
                )
                ...
                if original_yz_from_click:
                    ...
                    if SCIPY_AVAILABLE:
                        ...
                        distances_sq = np.sum(
                            (pc_slice_for_search[:, 1:3] - np.array([world_y_click, world_z_click]))
                            ** 2,
                            axis=1,
                        )
                ...
                point_b_display_suffix = f"B{coords!s}"

            if point_b_to_store:
                self.add_pair(self.temp_point_a, point_b_to_store)
                self.set_status(
                    f"Pair added: A{self.temp_point_a} <-> {point_b_display_suffix}. Click next point on Image A."
                )
                self.temp_point_a = None
                self.btn_clear_selection.config(state=DISABLED)
    ...
    def add_pair(self, point_a, point_b) -> None:
    ...
    def update_listbox(self) -> None:
        ...
        for i, (p_a, p_b) in enumerate(self.point_pairs):
            if self.is_b_point_cloud and isinstance(p_b, tuple) and len(p_b) == 3:
                ...
                pb_str = (
                    f"B({p_b[0]:.2f}, {p_b[1]:.2f})"
                    if isinstance(p_b[0], float) or isinstance(p_b[1], float)
                    else f"B{p_b!s}"
                )
    ...
    def _redraw_all_points(self) -> None:
        ...
        for i, (p_a, p_b) in enumerate(self.point_pairs):
            ...
            if self.image_a and self.canvas_a:
                ...
                transformed_unzoomed_ax, transformed_unzoomed_ay = (
                    self._transform_coords_from_original_to_display_a(p_a[0], p_a[1])
                )
            ...
            if self.image_b and self.canvas_b:
                ...
                if self.is_b_point_cloud and isinstance(p_b, tuple) and len(p_b) == 3:
                    ...
                    pixel_coords_b_on_heatmap = self._get_heatmap_pixel_from_original_yz(
                        p_b[1], p_b[2]
                    )
                    ...
                    isinstance(p_b, tuple) and len(p_b) == 2
                ):  # Regular image or legacy 2D point for PC
                ...
                if disp_bx_pixel_zoomed is not None and disp_by_pixel_zoomed is not None:
                    self._draw_marker(
                        self.canvas_b, disp_bx_pixel_zoomed, disp_by_pixel_zoomed, color, tag
                    )
        ...
        if self.temp_point_a and self.image_a and self.canvas_a:
            ...
            transformed_unzoomed_ax, transformed_unzoomed_ay = (
                self._transform_coords_from_original_to_display_a(
                    self.temp_point_a[0], self.temp_point_a[1]
                )
            )
            if transformed_unzoomed_ax is not None:
                disp_ax_zoomed = transformed_unzoomed_ax * self.zoom_level_a
                disp_ay_zoomed = transformed_unzoomed_ay * self.zoom_level_a
                self._draw_marker(
                    self.canvas_a, disp_ax_zoomed, disp_ay_zoomed, COLOR_TEMP, "point_marker"
                )

    def _draw_marker(self, canvas, x, y, color, tag) -> None:
        ...
        ix, iy = int(x), int(y)
        canvas.create_oval(
            ix - radius, iy - radius, ix + radius, iy + radius, outline=color, width=2, tags=tag
        )
    ...
    def highlight_pair(self, index, redraw_only=False) -> None:
    ...
    def clear_highlights(self, clear_temp=True) -> None:
    ...
    def scroll_to_view(self, canvas, x, y) -> None:
        ...
        if not scroll_region or not scroll_region.strip():
            return
        try:
    ...
    def remove_selected_pair(self) -> None:
        ...
        if self.current_selection_index is None or not (
            0 <= self.current_selection_index < len(self.point_pairs)
        ):
        ...
        if new_selection_index >= 0:
            ...
            self.set_status(
                f"Removed pair: A{removed_pair[0]} <-> B{removed_pair[1]}. Selected pair {new_selection_index + 1}."
            )
        else:
            self._redraw_all_points()
            self.set_status(
                f"Removed pair: A{removed_pair[0]} <-> B{removed_pair[1]}. List is now empty."
            )

    # --- Warp Calculation ---

    def calculate_warp(self) -> None:  # Renamed
        ...
        for pair_idx, (pa, pb) in enumerate(self.point_pairs):
            ...
            if self.is_b_point_cloud and isinstance(pb, tuple) and len(pb) == 3:
                ...
                isinstance(pb, tuple) and len(pb) == 2
            ):  # Standard 2D image or legacy 2D point for PC
                points_b_np_list.append(pb)
            else:
                messagebox.showerror(
                    "Warp Error", f"Invalid format for Point B in pair {pair_idx + 1}."
                )
                return
        ...
        try:
            if method == WARP_METHOD_HOMOGRAPHY:
                ...
                if matrix is None:
                    msg = "Homography calculation failed. Check point correspondences."
                    raise ValueError(msg)
                ...
                self.set_status(
                    f"Homography calculated successfully using {inlier_count}/{len(points_a_np)} inliers."
                )
                ...
                tps.estimateTransformation(
                    points_a_np.reshape(1, -1, 2), points_b_np.reshape(1, -1, 2), matches
                )
                self.tps_transformer = tps  # Store the A -> B transformer
                self.set_status(
                    f"TPS transformation calculated using {len(points_a_np)} points (for remap)."
                )
            ...
            messagebox.showerror(
                "Warp Error", f"An error occurred during {method} calculation:\n{e}"
            )
    ...
    def save_point_pairs(self) -> None:
        ...
        try:
            with open(filepath, "w", encoding="utf-8") as f:
                for pa, pb in self.point_pairs:
                    # pb can be 2-tuple or 3-tuple
                    pb_str_parts = [str(coord) for coord in pb]
    ...
    def load_point_pairs(self) -> None:
            ...
            with open(filepath, encoding="utf-8") as f:
                for i, line in enumerate(f):
                    ...
                    if not (4 <= len(parts) <= 5):  # Allow 4 (2D for B) or 5 (3D for B)
                        msg = f"Line {i + 1}: Each line must contain 4 or 5 coordinate values."
                        raise ValueError(msg)
    ...
    def load_homography_data(self) -> None:
        filepath = filedialog.askopenfilename(
        ...
        try:
            ...
            self._update_warp_button_states()
            self.set_status(
                f"Homography data loaded from {os.path.basename(filepath)}. Image A: {data.get('image_a_path', 'N/A')}, Image B: {data.get('image_b_path', 'N/A')}"
            )
            if self.alignment_window and self.alignment_window.winfo_exists():
                self.alignment_window.destroy()
        except Exception as e:
    ...
    def show_alignment(self) -> None:
        ...
        if not warp_calculated:
            messagebox.showerror(
                "Alignment Error", "Warp transform not calculated yet. Please calculate it first."
            )
            return
        ...
        try:
            ...
            if self.blended_pil_image is None:
                ...
                if (
                    self.warp_method.get() == WARP_METHOD_HOMOGRAPHY
                    ...
                    if self.is_b_point_cloud and self.heatmap_yz_min_max_b:
                        ...
                        s_y_coeff = (
                            delta_y / (HEATMAP_WIDTH - 1.0)
                            if delta_y != 0 and HEATMAP_WIDTH > 1
                            else 0
                        )
                        ...
                        s_z_coeff = (
                            -delta_z / (HEATMAP_HEIGHT - 1.0)
                            if delta_z != 0 and HEATMAP_HEIGHT > 1
                            else 0
                        )
                    ...
                    _grid_y, _grid_x = np.mgrid[0:h_a, 0:w_a].astype(np.float32)

                    # map_x will store Y-coords in B's space, map_y will store Z-coords in B's space (or X,Y for regular image)
                    map_b_coord1 = np.zeros(
                        (h_a, w_a), dtype=np.float32
                    )  # For Y_world or X_pixel_B
                    map_b_coord2 = np.zeros(
                        (h_a, w_a), dtype=np.float32
                    )  # For Z_world or Y_pixel_B
                    ...
                    for y in range(0, h_a, block_size):
                        ...
                        for x in range(0, w_a, block_size):
                            ...
                            for yi in range(y, y_end):
                                block_coords.extend([xi, yi] for xi in range(x, x_end))

                            block_coords = np.float32(block_coords).reshape(-1, 1, 2)

                            # Wende TPS auf den Block an
                            try:
                                transformed_coords = self.tps_transformer.applyTransformation(
                                    block_coords
                                )
                    ...
                    if self.is_b_point_cloud and self.heatmap_yz_min_max_b:
                        ...
                        if delta_y == 0 or HEATMAP_WIDTH <= 1:
                            map_x_heatmap_px = np.full_like(
                                map_b_coord1, (HEATMAP_WIDTH - 1.0) / 2.0
                            )
                        else:
                            map_x_heatmap_px = ((map_b_coord1 - y_min_data) / delta_y) * (
                                HEATMAP_WIDTH - 1.0
                            )

                        if delta_z == 0 or HEATMAP_HEIGHT <= 1:
                            map_y_heatmap_px = np.full_like(
                                map_b_coord2, (HEATMAP_HEIGHT - 1.0) / 2.0
                            )
                        else:
                            # map_b_coord2 contains Z_world. Heatmap Y pixel is (z_max - Z_world) / range * H
                            map_y_heatmap_px = ((z_max_data - map_b_coord2) / delta_z) * (
                                HEATMAP_HEIGHT - 1.0
                            )

                        map_x_heatmap_px = np.clip(map_x_heatmap_px, 0, HEATMAP_WIDTH - 1).astype(
                            np.float32
                        )
                        map_y_heatmap_px = np.clip(map_y_heatmap_px, 0, HEATMAP_HEIGHT - 1).astype(
                            np.float32
                        )
                        ...
                        warped_b = cv2.remap(
                            img_b_cv,
                            map_b_coord1.astype(np.float32),
                            map_b_coord2.astype(np.float32),
                            cv2.INTER_LINEAR,
                            borderMode=cv2.BORDER_CONSTANT,
                            borderValue=(0, 0, 0),
                        )
                    ...
                    messagebox.showerror(
                        "Alignment Error", "Warp transform not calculated or method mismatch."
                    )
                    return
                ...
                if warped_b.shape != img_a_cv.shape:
                    print(
                        f"Warning: Warped image shape {warped_b.shape} differs from target {img_a_cv.shape}. Resizing."
                    )
                    warped_b = cv2.resize(
                        warped_b,
                        (img_a_cv.shape[1], img_a_cv.shape[0]),
            ...
            if not (self.alignment_window and self.alignment_window.winfo_exists()):
                ...
                if self.is_b_point_cloud and self.last_filtered_points_for_heatmap_xyz is not None:
                    self.alignment_window_active_pc_slice_xyz = np.copy(
                        self.last_filtered_points_for_heatmap_xyz
                    )
                else:
                ...
                Button(
                    zoom_frame, text="+", width=2, command=lambda: self._zoom_alignment(ZOOM_STEP)
                ).pack(side=LEFT)
                Button(
                    zoom_frame,
                    text="-",
                    width=2,
                    command=lambda: self._zoom_alignment(1 / ZOOM_STEP),
                ).pack(side=LEFT)
                Button(
                    zoom_frame, text="100%", command=lambda: self._zoom_alignment(1.0, reset=True)
                ).pack(side=LEFT, padx=5)

                # Canvas Frame (will be added to PanedWindow)
                canvas_container_frame = Frame(
                    alignment_paned_window, width=img_w + 20
                )  # Initial width hint
                alignment_paned_window.add(
                    canvas_container_frame, weight=3
                ...
                self.alignment_canvas.bind(
                    "<ButtonPress-2>", lambda e: self.alignment_canvas.scan_mark(e.x, e.y)
                )
                self.alignment_canvas.bind(
                    "<B2-Motion>", lambda e: self.alignment_canvas.scan_dragto(e.x, e.y, gain=1)
                )
                ...
                listbox_container_frame = Frame(
                    alignment_paned_window, width=listbox_width_estimate
                )
                ...
                self.alignment_listbox = Listbox(
                    alignment_list_frame,
                ...
                self.btn_remove_alignment_point = Button(
                    alignment_buttons_frame,
                    text="Remove Selected Point",
                    command=self._remove_selected_alignment_point,
                    state=DISABLED,
                )
                ...
                Button(
                    ...
                    command=self._clear_alignment_points,
                ).pack(fill=X, pady=2)
                Button(
                    alignment_buttons_frame,
                    text="Save Alignment Points",
                    command=self._save_alignment_points,
                ).pack(fill=X, pady=2)
            ...
            messagebox.showerror(
                "Alignment Error", f"An error occurred during alignment visualization:\n{e}"
            )
            traceback.print_exc()  # Print traceback to console
            self.blended_pil_image = None
            if self.alignment_window and self.alignment_window.winfo_exists():
    ...
    def _on_alignment_close(self) -> None:
    ...
    def _zoom_alignment(self, factor, reset=False) -> None:
        """Zooms the alignment preview window."""
        if (
            not self.alignment_window
            or not self.alignment_window.winfo_exists()
            or not self.blended_pil_image
        ):
            return
    ...
    def _update_alignment_display(self) -> None:
        ...
        try:
            ...
            messagebox.showwarning(
                "Alignment Zoom Error", f"Could not resize alignment preview.\n{e}"
            )
            self.alignment_canvas.delete("all")
            self.alignment_canvas.config(scrollregion=(0, 0, 0, 0))
            self.tk_blended_image_display = None

    def _redraw_alignment_points(self) -> None:
        ...
        for i, (p_a, _p_b_data) in enumerate(self.alignment_point_pairs):
            ...
            self._draw_marker(
                self.alignment_canvas,
                disp_ax_zoomed,
                disp_ay_zoomed,
                color,
                "alignment_point_marker",
            )

    def _on_alignment_canvas_click(self, event) -> None:
        if not self.alignment_canvas or not self.blended_pil_image:
            return
        ...
        if self.warp_method.get() == WARP_METHOD_HOMOGRAPHY and self.homography_matrix is not None:
            try:
                ...
                if abs(transformed_pt_on_B_homogeneous[2]) > 1e-9:
                    img_b_orig_x = (
                        transformed_pt_on_B_homogeneous[0] / transformed_pt_on_B_homogeneous[2]
                    )
                    img_b_orig_y = (
                        transformed_pt_on_B_homogeneous[1] / transformed_pt_on_B_homogeneous[2]
                    )
            ...
            if (
                transformed_pt_on_B_tps_result is not None
                and len(transformed_pt_on_B_tps_result[0]) > 0
            ):
                # Handle potential tuple output from older OpenCV versions
                coords_array = (
                    transformed_pt_on_B_tps_result[0]
                    if isinstance(transformed_pt_on_B_tps_result, tuple)
                    else transformed_pt_on_B_tps_result
                )
        ...
        if self.is_b_point_cloud:
            if (
                self.heatmap_yz_min_max_b is None
                or self.alignment_window_active_pc_slice_xyz is None
            ):
            ...
            if pc_slice.shape[0] > 0:
                if SCIPY_AVAILABLE and KDTree is not None:
                    ...
                    _distance, index = kdtree.query([world_y, world_z])
                    nearest_3d_point = pc_slice[index]
                else:  # Fallback to brute force (slower)
                    distances_sq = np.sum(
                        (pc_slice[:, 1:3] - np.array([world_y, world_z])) ** 2, axis=1
                    )
                    index = np.argmin(distances_sq)
                    nearest_3d_point = pc_slice[index]
            ...
            point_b_data_to_store = (round(img_b_orig_x), round(img_b_orig_y))
        ...
        if point_b_data_to_store:
            ...
            self.set_status(
                f"Alignment point added: A({img_a_px},{img_a_py}) <-> {point_b_display_str}"
            )

    def _on_alignment_listbox_select(self, event) -> None:
        if not self.alignment_listbox or not self.alignment_listbox.curselection():
            if self.alignment_current_selection_index is not None:
                self.alignment_current_selection_index = None
                if self.btn_remove_alignment_point:
                    self.btn_remove_alignment_point.config(state=DISABLED)
        ...
        if selected_indices:
            ...
            if self.btn_remove_alignment_point:
                self.btn_remove_alignment_point.config(state=NORMAL)
            self._redraw_alignment_points()

    def _update_alignment_listbox(self) -> None:
        if not self.alignment_listbox or not (
            self.alignment_window and self.alignment_window.winfo_exists()
        ):
            return
    ...
    def _clear_alignment_points(self) -> None:
    ...
    def _remove_selected_alignment_point(self) -> None:
        if self.alignment_current_selection_index is None or not (
            0 <= self.alignment_current_selection_index < len(self.alignment_point_pairs)
        ):
            return
    ...
    def _save_alignment_points(self) -> None:
        ...
        try:
            with open(filepath, "w", encoding="utf-8") as f:
                f.write(
                    "# ImageA_X,ImageA_Y,ImageB_X_or_PC_X,ImageB_Y_or_PC_Y,(ImageB_Z_or_PC_Z if 3D)\n"
                )
                f.writelines(
                    f"{pa[0]},{pa[1]}," + ",".join(map(str, pb_data)) + "\n"
